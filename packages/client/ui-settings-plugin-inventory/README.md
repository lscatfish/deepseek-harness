...
Each card also carries an enablement switch that persists through `ctx.remote.pluginInventory.setEnabled()` and refreshes the catalog on success. While a switch is pending the row is disabled; a rejected or failed write surfaces a localized alert and leaves the row state untouched.
...
- **Toggle refreshes, does not predict** — the switch reflects the last fetched snapshot; after a successful write the tab refetches instead of optimistically flipping local state, so the applied state always comes from the Host.
