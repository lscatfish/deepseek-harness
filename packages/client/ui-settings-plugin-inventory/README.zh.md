...
每张卡片还带有一个启用开关，通过 `ctx.remote.pluginInventory.setEnabled()` 持久化，并在成功后刷新清单。开关处于待处理状态时该行被禁用；写入被拒绝或失败时会显示本地化的提示，且行状态保持不变。
...
- **开关刷新而非预测** —— 开关反映最近一次取得的快照；写入成功后标签页重新读取而不是乐观地翻转本地状态，因此呈现的状态始终来自 Host。
